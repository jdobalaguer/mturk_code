function createSdata(numbers) {
  ...
  var block;
  var dimsToChoose = shuffleArray([0, 1, 2]);
  var trainingDims = dimsToChoose.slice(0,2);
  var testDims = dimsToChoose.slice(2,3);
  for (var i_block=1; i_block<=(nb_blocks-1); i_block++) {
    block = createBlock(i_block,numbers,funcs[0],shapes[0], trainingDims);
  ...
  // set last block
  block = createBlock(nb_blocks,numbers,funcs[1],shapes[0], testDims);
  sdata.vb_block    = sdata.vb_block.concat(block.vb_block);
  sdata.vb_trial    = sdata.vb_trial.concat(block.vb_trial);
  sdata.vb_d1       = sdata.vb_d1.concat(block.vb_d1);
  sdata.vb_d2       = sdata.vb_d2.concat(block.vb_d2);
...
function createBlock(i_block,numbers,func,shape, dims) {
  var sdata = {};
  var nb_values = numbers.values;
  ...
  sdata.vb_d1 = replicateArray(dims[randi(dims.length-1)],nb_trials);
  sdata.vb_d2 = replicateArray(dims[randi(dims.length-1)],nb_trials);
  sdata.vb_x1 = xx[0];
  sdata.vb_x2 = xx[1];
...
function createY(xx,func) {
  ...
  for (var i = 0; i < xx[0].length; i++) {
    ...
    switch(func){
      case 0:
        var y0 = (normpdf(xx[0][i],0.5,sd)-normpdf(0,0.5,sd))/(normpdf(0.5,0.5,sd)-normpdf(0,0.5,sd));
        var y1 = (normpdf(xx[1][i],0.5,sd)-normpdf(0,0.5,sd))/(normpdf(0.5,0.5,sd)-normpdf(0,0.5,sd));
        y[i] = (y0 * y1);
        break;
      case 1:
